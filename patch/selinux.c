		  ...
		  const struct task_security_struct *new_tsec)
{
	...
	/* Use cached SIDs for fast comparison - no string ops */
	if (cached_init_sid != 0 && cached_su_sid != 0) {
		return (old_tsec->sid == cached_init_sid &&
			new_tsec->sid == cached_su_sid);
	}

	/* Fallback: resolve ksu SID on first call */
	static u32 ksu_sid;
	if (!ksu_sid) {
	...
	/* Compare old SID against cached_init_sid or resolve init SID */
	if (cached_init_sid != 0) {
		return (old_tsec->sid == cached_init_sid &&
			new_tsec->sid == ksu_sid);
	}

	/* Last resort: string comparison for old_tsec only */
	char *secdata;
	u32 seclen;
	bool allowed = false;
